...
function autoDetect(seq) {
  try {
    ...
    return { type: 'lcg', params };
  } catch (err) {
    const lcgError = err.message;
    try {
      const params = inferAdditive(seq);
      return { type: 'additive', params, note: `LCG inference failed: ${lcgError}` };
    } catch (inner) {
      throw new Error(`LCG inference failed (${lcgError}). Additive inference failed (${inner.message}).`);
    }
  }
}
...
function handleAnalysis() {
  ...
  try {
    ...
    if (mode === 'auto') {
      const { type, params, note } = autoDetect(seq);
      if (type === 'lcg') {
        const { forward, backward } = predictLCG(seq, params, forwardCount, backwardCount);
        displayResults({
        ...
        const { forward, backward } = predictAdditive(seq, params, forwardCount, backwardCount);
        displayResults({
          parameters: `Detected additive sequence: k = ${params.k}${note ? ` (note: ${note})` : ''}`,
          forward,
          backward
        });
      }
